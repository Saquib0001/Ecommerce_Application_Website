...
import pic from "../assets/Choclate_box.png";
import { useState } from 'react';

const Home = () => {

  const itemName = "Fantasie Chocolate"
  const itemPrice = 700
        ...
        mx-auto w-full my-16 border-2 bg-[#fcf6f6] border-slate-100 shadow-md py-4'>
           ...
           <div className='flex flex-col lg:w-6/12 w-full py-8'>
             ...
             <div className='flex text-slate-900 justify-center items-center mb-10'>
                ...
                py-2 text-5xl bg-red-200 cursor-pointer'>-</span>
                <span className='w-auto px-4 py-2 text-3xl font-semibold'>{quantity}</span>
                <span onClick={increment} className='select-none w-auto px-4 py-2 text-5xl bg-green-200 
                cursor-pointer'>+</span>
             </div>


              <div className='my-6 text-xl'>Amount to be paid:
              <span className='text-pink-500 text-3xl font-bold pl-3'>₹{finalAmount}</span></div>
              <div className='my-6'>
              <button onClick={checkout} className='bg-pink-500 text-white px-8 py-4 rounded-md text-2xl 
              font-semibold'>
                Checkout
              </button>
             </div>
